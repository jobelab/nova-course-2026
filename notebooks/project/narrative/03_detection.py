"""NARRATIVE COPY. Reads without data; computes nothing.

The interactive version of this chapter is one directory up and recomputes every
number below from the clouds. This copy is kept because it opens in a second and
needs none of the 9.6 GB the interactive one does.
...
def _(mo):
    ...
    **CHM detection is the wrong method for the ground-based clouds.** TLS and MLS score
    ...
    These rates come from one sampled run. The smallest quartile holds about 18 stems,
    so a single tree moves it by five points; the headline F1 values are stable across
    reruns but the per-quartile percentages wobble a little.
